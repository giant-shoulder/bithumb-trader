...
COIN_BLACKLIST = {'USDT', 'USDC', 'DAI', 'BUSD', 'TUSD', 'FDUSD',  # 스테이블 코인 제외
                  ...
                  'ZBT',                                              # 반복 손절 -1.88%/-3.59% (2026-04-25/26)
                  'GNO',                                              # ws_surge 급등→급락 패턴, 3전 1승2패 누적 -1.37% (2026-06-03)
                  'GWEI',                                             # 저가(256원) 슬리피지 손절 -3.20% (2026-06-09)
                  'AQT',                                              # 저유동성 슬리피지 손절 -3.32% (2026-06-08)
...
# ===== 최소 가격 / 스프레드 =====
# 300원 미만: 1원 틱이 손절폭 0.8% 대비 0.33% 이상 → 2~3틱 오차로 설계값보다 더 크게 손절됨
# 200원→300원: GWEI(256원) -3.20% 하드손절 원인, 저가 코인 슬리피지 피해 반복 방지 (2026-06-09 회고)
MIN_PRICE_KRW = 300          # 최소 코인 가격 (원) - 틱 크기 대비 손절 정밀도 보장 기준
MAX_SPREAD_PCT = 1.0         # 매수 허용 최대 스프레드 (%) - 2%→1%: 체결 슬리피지 강화 (2026-06 회고)

# ===== 매도 후 재매수 금지 시간 =====
COOLDOWN_AFTER_STOP_LOSS = 3600    # 손절 후 쿨다운 (초) - 1시간
COOLDOWN_AFTER_TAKE_PROFIT = 7200  # 익절 후 쿨다운 (초) - 2시간 (4시간→2시간: 당일 재진입 기회 확보, 2026-06 회고)
...
# ===== 최소 보유시간 =====
MIN_HOLD_SECONDS = 300        # AT 노이즈 청산 발동 최소 보유시간 (초) - 5분 = 1캔들 (600→300: VVV처럼 매수 직후 yellow 전환 시 18분 손실 방치 방지)
# 시간 기반 강제청산(MAX_HOLD_MINUTES) 제거 (2026-06-15): 하방은 손절가가 막고 추세 꺾임은 AT red가
# 잡으므로, 시계만 보고 손실 포지션을 자르면 반등 여지를 없앰. (기존 코드도 AT 노이즈 블록 continue로
# 사실상 미작동 상태였음)

# ===== 트레일링 스탑 =====
TRAILING_STOP_TRIGGER_PCT = 1.2  # 트레일링 스탑 발동 수익률 (%) - 이 이상 수익 시 추적 시작 (0.5→1.2: 슬리피지 > 여유폭 문제, 익절 구간 진입 후 추적, 2026-06 회고)
TRAILING_STOP_TRAIL_PCT = 0.4    # 고점 대비 추적 손절폭 (%) - 고점에서 이만큼 빠지면 청산

# ===== 익절 극대화 (2026-06-11) =====
# 본전 스탑: 0%~트레일링 발동(1.2%) 사이 무방비 구간 제거.
# +0.5% 도달 시 손절가를 매수가 +0.2%로 상향 → 수익 갔던 포지션이 풀 손절로 끝나는 것 방지
# (+0.2% = 왕복 수수료 0.08% + 슬리피지 여유 → 발동해도 순손익 양수)
BREAKEVEN_TRIGGER_PCT = 0.8    # 본전 스탑 발동 수익률 (%) - 0.5→0.8: 28초 스파이크 1번에 활성화 + 슬리피지로 마이너스 청산 방지 (2026-06-12 NEAR)
BREAKEVEN_STOP_PCT = 0.2       # 본전 스탑 위치 (매수가 대비 %)
BREAKEVEN_MIN_TICKS = 2        # 본전 스탑 최소 폭 (틱) - 저가 코인에서 % 버퍼가 틱 절사로 증발하는 것 방지 (2026-06-12 ONDO: 537원 코인 0.2%=1.07원 < 1틱)

# 1차 부분 익절: R:R 1.0 도달 시 절반 익절 확정, 잔량은 고정 익절가 해제 후 트레일링 러너
# (기존: R:R 1.5 전량 매도 → 급등 코인에서도 +1.3% 수준에서 차단되는 문제 해결)
PARTIAL_TP_RR = 1.0            # 1차 부분 익절 R:R (리스크 1배 도달 시)
PARTIAL_TP_RATIO = 0.5         # 1차 부분 익절 매도 비율 (잔량은 트레일링 추적)
...
# 소액 포지션 방지
MIN_BUY_KRW = 10_000         # 최소 실제 매수금액 (원) - 잔고 부족 시 소액 포지션 차단
