...
COIN_BLACKLIST = {'USDT', 'USDC', 'DAI', 'BUSD', 'TUSD', 'FDUSD',  # 스테이블 코인 제외
                  'GOAT', 'XVS', 'XTER',                            # 반복 손절 코인 (2026-03-30)
...
# ===== 손절/익절 (고정값, 단순) =====
HARD_STOP_PCT = 1.5           # 하드 손절 (%) - 스프레드+슬리피지 감안 확대 (0.8→1.5)
HARD_STOP_MIN_HOLD_SECONDS = 45  # 하드 손절 최소 보유시간 (초) - 매수 직후 즉시 손절 방지
TRAILING_ACTIVATE_PCT = 1.0   # 트레일링 스탑 활성화 수익 기준 (0.5→1.0%)

# 티어드 트레일링 스탑: 수익이 클수록 더 넓게 추격
# (수익 하한, 트레일링 폭) - 위에서부터 매칭
TRAILING_TIERS = [
    (5.0, 2.0),   # +5% 이상 수익 → 고점 대비 -2.0% 허용
    (2.0, 1.0),   # +2% 이상 수익 → 고점 대비 -1.0% 허용
...
COOLDOWN_AFTER_TAKE_PROFIT = 1800  # 익절 후 쿨다운 (초) - 30분 (10→30, 재진입 손실 방지)
...
DAILY_COIN_STOP_LIMIT = 2     # 코인별 일일 손절 한도 (초과 시 당일 블랙리스트)
